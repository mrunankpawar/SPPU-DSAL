...
class doubleHashTable:
    ...
    #Searches for an element in the table and returns position of element if found ekse returns False
    def search(self, record):
        ...
        # then we search element using double hashing 
            ...
            while i <= limit:
                ...
                self.comparisons += 1
                #if element at new position is equal to the required element 

                if(self.table[position] != None):
                    if self.table[position].get_name() == None:
                        found = True
                        break
